				...
				<div class="data-box">
					<div class="data1">
						<div class="data_contents btn-duration" data-duration="7"
							<%-- onclick="location.href='<%=request.getContextPath() %>/t/Helptest.do/${detail.midx}/7/${start}/${end}'" --%>
						...
						<div class="data_contents btn-duration" data-duration="30"
							<%-- onclick="location.href='<%=request.getContextPath() %>/t/Helptest.do/${detail.midx }/30/${start}/${end}'" --%>
							onclick="countsearch(this);">
						...
						<div class="data_contents btn-duration" data-duration="365"
							<%-- onclick="location.href='<%=request.getContextPath() %>/t/Helptest.do/${detail.midx }/365/${start}/${end}'" --%>
							onclick="countsearch(this);">
				...
				<div class="fixed-area" id="fixed-area">
					...
					<div class="date_box flex flex-center light" id="date_box">
						<div class="date flex flex-center">
							<input class="" type="date" value="<%=startDate %>" name="start"
								id="start">
						</div>
						...
						<!-- !!!! -->
						<div class="date flex flex-center">
							<input class="" type="date" value="<%=endDate %>" name="end" id="end"
								max="${maxDate}">
						</div>
...
function countsearch(element){		// btn : onclick="countsearch(this)"
    ...
    var duration = element.getAttribute("data-duration");		// elemnt는 그 요소의 data-duration 값을 반환 하는 것이기 때문에 selectBox에는 data-duration 값이 없어서 오류가 발생한다 
...
//	console.log("duration : " + duration);
	...
	$.ajax({
		type : 'post',
		url : "<%=request.getContextPath() %>/t/ListAjax",
		datatype : "json",
		data : {
			"midx" : midx,
			"start" : start,
	...
	//			console.log("reslist 길이"+ helplist.length);
				...
				var url = "<%= request.getContextPath() %>/h/Insert.do?h_name='help.h_name'&h_no='help.h_no'&h_number='help.h_number'";
		...
		//				var monthC = ('0' + (dateC.getMonth() + 1)).slice(-2);
						// getDate( )는 해당 월의 날짜를 반환
						var day = ('0' + date.getDate()).slice(-2);
		//				var dayC = ('0' + dateC.getDate()).slice(-2);
						// getHours()는 시간(24시간 형식)을 반환
						var hours = ('0' + date.getHours()).slice(-2);
						// getMinutes()는 분을 반환
						var minutes = ('0' + date.getMinutes()).slice(-2);
						var formattedDateTime = year + '.' + month + '.' + day + ' ' + hours + ':' + minutes;
		//				var formatDate = yearC + '-' + monthC + '-' + dayC; 
		//				var formatDate = year + '-' + month + '-' + day; 
		//				console.log("qq : "+formattedDateTime);		// 2023.07.06 09:37
		
			
		//				console.log("Ajax Date : " + formatDate);
		...
		//	console.log("jsp 페이지 start : "+start);
		}
	});
}
</script>
