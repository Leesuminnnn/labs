...
<link rel="stylesheet" type="text/css" href="<%=request.getContextPath() %>/resources/css/Pointcss.css">
<title>포인트 내역</title>
</head>
<style>
...
<script src="<%=request.getContextPath() %>/resources/js/jquery.nice-select.js"></script>
<script>
	...
	<div class="container">
		...
		<div class="gray-box">
			...
			<div class="date_box flex flex-center light">
				<div class="date flex flex-center">
					<input class="" type="date" value="${start}" name="start" id="start" oninput="startchange()">
				</div>
				...
				<div class="date flex flex-center">
					<input class="" type="date" value="${end}" name="end" id="end" oninput="endchange()" max="${maxDate}">
				</div>
		...
		<div class="status">
			<div class="area-custom-select">
				...
				<select class="select-origin btn-duration" data-duration="0" style="" name="select" id="select" onchange="">
					<option class="btn-duration" value="selectOption1" data-duration="" <c:if test="${select == 'selectOption1' }">selected</c:if>>전체</option>
					<option class="btn-duration" value="selectOption2" data-duration="" <c:if test="${select == 'selectOption2' }">selected</c:if>>적립</option>
					<option class="btn-duration" value="selectOption3" data-duration="" <c:if test="${select == 'selectOption3' }">selected</c:if>>사용</option>
				</select>
...
//    console.log(select);
    $.ajax({
    	...
    	success : function(response){
    		durationInput = null;
    		console.log("duration 인풋 변경 전 : "+durationInput);
			
    		durationInput = duration;
			console.log("duration 인풋 변경 후 : "+durationInput);
...
function handleClick(duration) {
    ...
    xhr.open("GET", "Point.do", true);
    xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    xhr.onreadystatechange = function() {
        if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
            // 서버로부터 응답 받은 내용을 처리합니다.
        	
        }
    };
    xhr.send("duration=" + duration);

    // duration 값을 세션에 저장합니다.
    var data = new FormData();
    data.append('duration', duration);
    fetch('SetSessionDuration.do', {
        method: 'POST',
        body: data
    });
}
...
/* 
// 모든 버튼 요소를 선택합니다.
var buttons = document.querySelectorAll('.btn_box .btn-duration');
...
function handleClick(element) {
	...
	var xhr = new XMLHttpRequest();
	xhr.open("GET", "${pageContext.request.contextPath}/m/Point.do/${detail.midx}/" + duration + "/${start}/${end}", true);
	xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	xhr.onreadystatechange = function() {
		if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
		// 서버로부터 응답 받은 내용을 처리합니다.
...
function endchange(){
	let value = document.getElementById('end').value;
	console.log(value);
	this.value = value;
}
...
function search() {
	...
	var selectInput = $("#select").val();
	var spantext = $('#current').text();
	theForm.action="${pageContext.request.contextPath}/m/Point.do/${detail.midx }?start=${start}&end=${end}&select="+selectInput;
	theForm.submit();
}
 */
 
// jquery.nice-select.js 에서 연결 되는 부분
// selectbox 변경되면 실행 되게
...
$('#select').change(function(){
	console.log("test : " + $("#select option:selected").val());
});
 */
